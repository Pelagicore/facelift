...
explicitly in your C++ implementation of the interface (derived from IReadyPropertyAdaptor in the
...
\code
    m_readyProp = 0;                    // default value, as we don't know better yet
    m_readyProp.setReadiness(false);    // explicitely flag that the value is not properly set yet
    QTimer::singleShot(1000, this, [this] { m_readyProp = 42; });    // assignment will set readiness to true automatically
\endcode
...
IReadyFrontend {    // or whatever name the frontend is registered with
    onReadinessChanged: console.log("Readiness of readyProp is " + readiness.readyProp);
    onReadyPropChanged: console.log("Value of readyProp changed to " + readyProp);
}
\endcode
