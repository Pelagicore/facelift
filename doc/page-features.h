...
By default the \c readyFlag value will be true. It is however possible to set it to false
explicitly in your C++ implementation of the interface (derived from IReadyPropertyAdapter in the
example above). On the property you can call \c setReadyFlag(false) and set some reasonable
default value before. Note that whenever the poperty is assigned or bound its readyFlag will
automatically switch to true. It can be set explicetely to true, as well, of course
(\c setReadyFlag(true)). The following code snippet, if added to the constructor simulates a
property that becomes available after 1s:
\code
    m_someProperty = 0;                    // default value, as we don't know better yet
    m_someProperty.setReadyFlag(false);    // explicitely flag that the value is not properly set yet
    QTimer::singleShot(1000, this, [this] { m_someProperty = 42; });    // assignment will set readyFlag to true automatically
\endcode
...
\c readyFlags property and its elements are ready-only, of course. The following code snippet shows
...
you will not get indivdual signals for the readyFlag of each of the properties, but get a
\c readyFlagsChanged signal whenever the readyFlag of a single property changes.
